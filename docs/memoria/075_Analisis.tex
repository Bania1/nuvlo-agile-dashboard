...
\bottomrule
\endfoot
\texttt{usuario\_autenticado} & \texttt{id}, \texttt{atlassian\_account\_id}, \texttt{email}, \texttt{display\_name}, \texttt{created\_at}, \texttt{updated\_at}. & Identifica al usuario mediante Atlassian. No almacena contraseña local, ya que la autenticación se delega en el proveedor externo. \\
\texttt{sesion\_atlassian} & \texttt{id}, \texttt{usuario\_id}, \texttt{cloud\_id}, \texttt{site\_name}, \texttt{site\_url}, \texttt{encrypted\_access\_token}, \texttt{encrypted\_refresh\_token}, \texttt{scopes}, \texttt{expires\_at}, \texttt{created\_at}, \texttt{updated\_at}. & Guarda la autorización concedida por Atlassian. Los \textit{tokens} se almacenan cifrados y se asocian al sitio Jira autorizado. \\
...
\texttt{proyecto\_jira} & \texttt{id}, \texttt{cloud\_id}, \texttt{jira\_id}, \texttt{key}, \texttt{name}, \texttt{avatar\_url}. & Conserva la información mínima del proyecto sincronizado desde Jira Cloud. La combinación \texttt{cloud\_id} y \texttt{jira\_id} evita duplicados entre sitios. \\
\texttt{tablero\_jira} & \texttt{id}, \texttt{jira\_id}, \texttt{name}, \texttt{type}, \texttt{project\_id}. & Representa tableros Scrum o Kanban asociados a un proyecto importado. \\
\texttt{sprint} & \texttt{id}, \texttt{jira\_id}, \texttt{name}, \texttt{state}, \texttt{start\_date}, \texttt{end\_date}, \texttt{complete\_date}, \texttt{board\_id}. & Permite agrupar incidencias por iteración y calcular métricas por periodo cuando Jira proporciona esa información. \\
