...
Redis se utiliza como almacén temporal para cachés de corta duración, estados de sincronización y datos auxiliares con caducidad automática~\cite{RedisDocs}. El estado OAuth, el verificador PKCE y la protección CSRF se gestionan mediante \textit{cookies} temporales y cabeceras HTTP, mientras que Redis queda reservado para información operativa que puede regenerarse si se pierde.
...
\midrule
Datos efímeros & Redis & TTL nativo para cachés temporales y estado de sincronización; OAuth/PKCE/CSRF se apoyan en \textit{cookies} temporales. \\
\midrule
